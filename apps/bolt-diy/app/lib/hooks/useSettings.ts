...
export function useSettings(): UseSettingsReturn {
  ...
  const [settings, setSettings] = useState<Settings>(() => {
    ...
    return {
      ...
      tabConfiguration,
      editor: {
        minimap: storedSettings?.editor?.minimap ?? true,
        codeFolding: storedSettings?.editor?.codeFolding ?? true,
        fontSize: storedSettings?.editor?.fontSize ?? 14,
        tabSize: storedSettings?.editor?.tabSize ?? 2,
        formatOnSave: storedSettings?.editor?.formatOnSave ?? true,
      },
    };
  });
  ...
  const updateEditorSettings = useCallback(
    (newEditorSettings: Partial<EditorSettings>) => {
      saveSettings({ editor: { ...settings.editor, ...newEditorSettings } });
    },
    [saveSettings, settings.editor],
  );
